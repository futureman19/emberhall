...
const out='art/verification/civic/player-scissor-diagnostic';fs.mkdirSync(out,{recursive:true});assert(!fs.existsSync(out+'/results.json'));
...
try {
for(const device of ['mobile']) for(const isolated of [false]) {
...
result.checks.push({device,isolated,bounds:evidence.bounds,unobstructedEqual:evidence.unobstructedEqual,meshes:evidence.meshes,restoredEqual:evidence.restoredEqual,passed:evidence.meshes>0&&evidence.restoredEqual&&(isolated?evidence.unobstructedEqual:!evidence.unobstructedEqual)});save();await context.close();
