...
const out='art/verification/civic/player-targeted-depth-cost';fs.mkdirSync(out,{recursive:true});assert(!fs.existsSync(out+'/results.json'));
...
const evidence=await page.evaluate(async(isolated)=>{const source=await(await fetch('/src/components/game/terrain.tsx')).text();const match=source.match(/from\s+["']([^"']*\/three\.js[^"']*)["']/);if(!match)throw Error('Exact Three import missing');const T=await import(match[1]);
 ...
 for(const c of copies)scene.add(c);gl.render(scene,camera);const candidate=gl.domElement.toDataURL(); const previousLoop=window.__settlementQA.state.frameloop;window.__settlementQA.state.setFrameloop('never');const timings=[];
 try{for(const enabled of [false,true,true,false]){const samples=[];for(let n=0;n<24;n++){await new Promise(requestAnimationFrame);for(const copy of copies)copy.visible=enabled;const begin=performance.now();if(enabled){for(const copy of copies)copy.visible=false;player.visible=false;gl.setRenderTarget(target);{const override=scene.overrideMaterial,auto=gl.shadowMap.autoUpdate;scene.overrideMaterial=depthMaterial;gl.shadowMap.autoUpdate=false;try{gl.render(scene,camera)}finally{scene.overrideMaterial=override;gl.shadowMap.autoUpdate=auto}}player.visible=visible;gl.setRenderTarget(oldTarget);for(const copy of copies)copy.visible=true;}gl.render(scene,camera);gl.getContext().finish();samples.push(performance.now()-begin);}timings.push({enabled,samples});}}finally{window.__settlementQA.state.setFrameloop(previousLoop);for(const copy of copies)copy.visible=true;}
for(const c of copies){scene.remove(c);c.material.dispose()}gl.render(scene,camera);const restoredEqual=before===gl.domElement.toDataURL();for(const [o,v]of hidden)o.visible=v;depthMaterial.dispose();target.dispose();target.depthTexture.dispose();return{timings,before,candidate,restoredEqual,unobstructedEqual:before===candidate,meshes:copies.length};
},isolated);
