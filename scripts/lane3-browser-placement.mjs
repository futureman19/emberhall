...
const out='art/lane3/browser-placement';fs.mkdirSync(out,{recursive:true});assert(!fs.existsSync(out+'/results.json'));
const kinds=['dormitory','kitchen','yard','market','forge','tavern','notice','board','farm'];
const result={scope:'Local8093 renderer, missing-kind disposable worlds, real build-state/useTile dispatch; not pointer input, physical mobile or animation acceptance.',rows:[],errors:[]};const save=()=>fs.writeFileSync(out+'/results.json',JSON.stringify(result,null,2));
...
try{for(const kind of kinds){
 ...
 row.after=await p.evaluate(({kind,site})=>{const s=window.__ember.useGame.getState();s.useTile(site.tx,site.ty);const w=window.__ember.getWorld();return {gold:w.gold,count:w.buildings.length,building:w.buildings.find(b=>b.kind===kind),buildKind:window.__ember.useGame.getState().buildKind};},{kind,site:setup.site});
 row.commitPassed=row.after.count===setup.count+1&&row.after.gold===100-(kind==='dormitory'?40:28)&&row.after.building?.tx===setup.site.tx&&row.after.building?.ty===setup.site.ty&&row.after.buildKind===null;save();assert(row.commitPassed,'Placement commit '+kind);await c.close();
