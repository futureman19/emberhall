...
const out='art/verification/civic/tavern-preflora-access';fs.mkdirSync(out,{recursive:true});assert(!fs.existsSync(out+'/results.json'));
const result={scope:'Sampled normal-speed QA useTile route256,298 to262,302, unchanged terrain, repositioned start. Screenshots perturb timing; no performance or continuous visibility claim.',checks:[],errors:[]};
...
await page.goto('https://emberhall-vale-4hukuk6ze-andrews-projects-ffe8a9fd.vercel.app/?qa=1');await page.getByRole('button',{name:'Continue',exact:true}).click();await page.waitForFunction(()=>window.__emberCamera&&window.__ember?.useGame.getState().phase==='playing');
...
const before=await state();await page.evaluate(()=>window.__ember.useGame.getState().useTile(262,302));const intent=await state();
const samples=[];await page.evaluate(()=>window.__ember.useGame.getState().speed(1));
for(let i=0;i<12;i++){await page.waitForTimeout(400);const position=await state();const file=device+'-'+i+'.png';await page.screenshot({path:out+'/'+file});samples.push({position,file});}
await page.evaluate(()=>window.__ember.useGame.getState().speed(0));const after=await state();
result.checks.push({device,before,intent,after,samples,passed:intent.path.length>0&&Math.hypot(after.x-262,after.z-302)<.25});save();await context.close();
}
