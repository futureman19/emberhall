...
const [url='http://127.0.0.1:8123',label='vis-u3-melee-dev',mode='candidate']=process.argv.slice(2);
...
const result={mode,url,scope:'Five equipment fixture swaps and rendered attachments; desktop/mobile viewport. No combat, touch interaction, ghost or complete animation acceptance.',cases:[],errors:[]};
...
try {
 for(const [device,viewport] of [['desktop',{width:1440,height:960}],['mobile',{width:390,height:844}]]) {
  ...
  if(mode==='fallback')await context.route('**/art/lanternwood/weapons.glb',r=>r.fulfill({status:503,body:'Deliberate local QA rejection'}));
  ...
  for(const weapon of ['knife','sword','club','mace','staff']) {
   await page.evaluate(weapon=>{const w=window.__ember.getWorld(),s=window.__ember.useGame.getState();w.player.wear.main=weapon;s.tick(.01);},weapon);await page.waitForTimeout(400);
   const row={device,weapon};
   if(mode!=='built'){
    row.parts=await page.evaluate(async()=>{const text=await(await fetch('/src/components/game/world-scene.tsx')).text(),f=text.match(/from\s+["']([^"']*(?:react-three_fiber|@react-three\/fiber)[^"']*)["']/),fiber=await import(f[1]);const scene=fiber._roots.get(document.querySelector('canvas')).store.getState().scene;const parts=[];scene.traverse(o=>{if(o.geometry?.userData.weaponPart)parts.push(o.geometry.userData.weaponPart);});return parts.sort();});
    assert.equal(row.parts.length,mode==='fallback'?0:weapon==='sword'?3:2);
   }
