...
const out='art/verification/civic/player-scenery-depth-cost';fs.mkdirSync(out,{recursive:true});assert(!fs.existsSync(out+'/results.json'));
...
const evidence=await page.evaluate(async(isolated)=>{const source=await(await fetch('/src/components/game/terrain.tsx')).text();const match=source.match(/from\s+["']([^"']*\/three\.js[^"']*)["']/);if(!match)throw Error('Exact Three import missing');const T=await import(match[1]);
 ...
 try{for(const enabled of [false,true,true,false]){const samples=[];for(let n=0;n<24;n++){await new Promise(requestAnimationFrame);for(const copy of copies)copy.visible=enabled;const begin=performance.now();if(enabled){for(const copy of copies)copy.visible=false;player.visible=false;gl.setRenderTarget(target);gl.render(scene,camera);player.visible=visible;gl.setRenderTarget(oldTarget);for(const copy of copies)copy.visible=true;}gl.render(scene,camera);gl.getContext().finish();samples.push(performance.now()-begin);}timings.push({enabled,samples});}}finally{window.__settlementQA.state.setFrameloop(previousLoop);for(const copy of copies)copy.visible=true;}
