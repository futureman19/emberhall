...
const root=new URL('../',import.meta.url),out=new URL('art/verification/commons/live/',root);fs.mkdirSync(out,{recursive:true});
const fixture=JSON.parse(fs.readFileSync(new URL('public/art/commons-review-save.json',root),'utf8'));
const local=JSON.parse(fs.readFileSync(new URL('art/verification/commons/candidate/results.json',root),'utf8'));
...
for(const [device,viewport] of [['desktop',{width:1440,height:960}],['mobile',{width:390,height:844}]]){
 ...
 for(const kind of ['dormitory','yard','farm']){
  ...
  const waypoint=local.checks.find(c=>c.name===device+'-'+kind+'-court-waypoint')?.target??[256,298];
  await walk(device+'-'+kind+'-court',waypoint);
  ...
  const before=await state();await page.mouse.click(pts[0].x,pts[0].y);await page.waitForTimeout(350);const after=await state();
  check(device+'-'+kind+'-existing-station-contract',kind==='yard'?!!after.openCraft:!after.openCraft);
  if(kind==='yard')await snap(device+'-yard-bench-ui');
