...
const out='art/verification/civic/player-depth-no-shadow-refresh';fs.mkdirSync(out,{recursive:true});assert(!fs.existsSync(out+'/results.json'));
const result={scope:'Frozen local renderer cost ABBA, 24 frames/block, requestAnimationFrame paced with gl.finish synchronization. Includes CPU plus forced GPU completion, not production FPS. R3F automatic rendering paused during samples.',checks:[],errors:[]};
...
const evidence=await page.evaluate(async(isolated)=>{const source=await(await fetch('/src/components/game/terrain.tsx')).text();const match=source.match(/from\s+["']([^"']*\/three\.js[^"']*)["']/);if(!match)throw Error('Exact Three import missing');const T=await import(match[1]);
 ...
 const size=gl.getDrawingBufferSize(new T.Vector2()),target=new T.WebGLRenderTarget(size.x,size.y);target.depthTexture=new T.DepthTexture(size.x,size.y);const oldTarget=gl.getRenderTarget(),visible=player.visible;player.visible=false;gl.setRenderTarget(target);{const auto=gl.shadowMap.autoUpdate;gl.shadowMap.autoUpdate=false;try{gl.render(scene,camera)}finally{gl.shadowMap.autoUpdate=auto}}player.visible=visible;gl.setRenderTarget(oldTarget);
 ...
 for(const c of copies)scene.add(c);gl.render(scene,camera);const candidate=gl.domElement.toDataURL();
 const previousLoop=window.__settlementQA.state.frameloop;window.__settlementQA.state.setFrameloop('never');const timings=[];
 try{for(const enabled of [false,true,true,false]){const samples=[];for(let n=0;n<24;n++){await new Promise(requestAnimationFrame);for(const copy of copies)copy.visible=enabled;const begin=performance.now();if(enabled){for(const copy of copies)copy.visible=false;player.visible=false;gl.setRenderTarget(target);{const auto=gl.shadowMap.autoUpdate;gl.shadowMap.autoUpdate=false;try{gl.render(scene,camera)}finally{gl.shadowMap.autoUpdate=auto}}player.visible=visible;gl.setRenderTarget(oldTarget);for(const copy of copies)copy.visible=true;}gl.render(scene,camera);gl.getContext().finish();samples.push(performance.now()-begin);}timings.push({enabled,samples});}}finally{window.__settlementQA.state.setFrameloop(previousLoop);for(const copy of copies)copy.visible=true;}
 for(const c of copies){scene.remove(c);c.material.dispose()}gl.render(scene,camera);const restoredEqual=before===gl.domElement.toDataURL();for(const [o,v]of hidden)o.visible=v;target.dispose();target.depthTexture.dispose();return{timings,before,candidate,restoredEqual,unobstructedEqual:before===candidate,meshes:copies.length};
},isolated);
...
result.checks.push({device,isolated,timings:evidence.timings,unobstructedEqual:evidence.unobstructedEqual,meshes:evidence.meshes,restoredEqual:evidence.restoredEqual,passed:evidence.meshes>0&&evidence.restoredEqual&&(isolated?evidence.unobstructedEqual:!evidence.unobstructedEqual)});save();await context.close();
