...
const out='art/verification/civic/player-silhouette-negative-control';fs.mkdirSync(out,{recursive:true});assert(!fs.existsSync(out+'/results.json'));
const result={scope:'Isolated-player negative control: hide non-player meshes only in synchronous disposable render, preserving live scene afterward. Tests self-occlusion leakage, not a proposal to remove scenery.',checks:[],errors:[]};
...
const evidence=await page.evaluate(()=>{
 ...
 const hidden=[];scene.traverse(o=>{if(!o.isMesh)return;let isPlayer=false;for(let p=o;p;p=p.parent)if(p===player)isPlayer=true;if(!isPlayer){hidden.push([o,o.visible]);o.visible=false;}});
