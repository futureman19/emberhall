...
const label=process.argv[3]??'hall-draw-census';assert.match(label,/^[a-z0-9-]+$/);const out='art/verification/civic/'+label+'.json';assert(!fs.existsSync(out),'Preserve previous results');
const result={scope:'Current civic renderer in both arms, mobile hall interior, fixed fixture/noon/clear. Off aborts only new Phase1 props/signs. CPU render submission is NOT GPU execution time. Diagnostic does not waive original failed gates.',order:['on'],frames:300,settleMs:8500,runs:[],errors:[]};
...
try {
 for(const arm of result.order){
  ...
  const census=await page.evaluate(()=>new Promise(resolve=>{const g=window.__perf.gl,render=g.render,direct=g.renderBufferDirect,rows=[];g.renderBufferDirect=function(camera,scene,geometry,material,object,group){const before={...g.info.render};const value=direct.call(this,camera,scene,geometry,material,object,group);const path=[];for(let n=object;n;n=n.parent)path.push(n.name||n.type);rows.push({path:path.reverse().join('/'),geometry:geometry.type,material:material.type,instances:object.isInstancedMesh?object.count:1,triangles:g.info.render.triangles-before.triangles,calls:g.info.render.calls-before.calls});return value};g.render=function(...args){try{return render.apply(this,args)}finally{g.render=render;g.renderBufferDirect=direct;resolve({rows,totals:{...g.info.render}})}}}));
  result.runs.push({arm,...census});flush();await ctx.close();
 }
...
console.log(JSON.stringify({passed:result.passed,draws:result.runs[0]?.rows.length,failure:result.failure}));
