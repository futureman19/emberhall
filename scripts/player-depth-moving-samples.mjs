...
const out='art/verification/civic/player-depth-moving-samples';fs.mkdirSync(out,{recursive:true});assert(!fs.existsSync(out+'/results.json'));
const result={scope:'Six normal-speed route samples per viewport; overlay reconstructed per sample, not persistent-frame synchronization or continuous animation acceptance. Temporary invisible test occluder retained; terrain unchanged.',checks:[],errors:[]};
...
try {
for(const device of ['desktop','mobile']) for(const isolated of [false]) {
...
for(let sample=0;sample<6;sample++){await page.waitForTimeout(600);
const evidence=await page.evaluate(async(isolated)=>{const source=await(await fetch('/src/components/game/terrain.tsx')).text();const match=source.match(/from\s+["']([^"']*\/three\.js[^"']*)["']/);if(!match)throw Error('Exact Three import missing');const T=await import(match[1]);
 ...
 const c=o.clone(false);c.material=o.material.clone();c.material.color?.set('#ffcf70');c.material.emissive?.set('#ffcf70');c.material.emissiveIntensity=1;c.material.transparent=true;c.material.opacity=.55;c.material.depthFunc=6;c.material.depthTest=false;c.material.onBeforeCompile=shader=>{shader.uniforms.sceneDepth={value:target.depthTexture};shader.uniforms.depthSize={value:size};shader.fragmentShader='uniform sampler2D sceneDepth;uniform vec2 depthSize;\n'+shader.fragmentShader;shader.fragmentShader=shader.fragmentShader.replace('void main() {','void main() { if(gl_FragCoord.z<=texture2D(sceneDepth,gl_FragCoord.xy/depthSize).x+0.000001) discard;');};c.material.customProgramCacheKey=()=> 'scenery-depth-spike';c.material.depthWrite=false;c.material.toneMapped=false;c.matrixAutoUpdate=false;c.matrix.copy(o.matrixWorld);c.userData.sourceMatrix=o.matrixWorld.toArray();c.renderOrder=10000;c.raycast=()=>{};c.castShadow=false;c.receiveShadow=false;copies.push(c);
 });
 for(const c of copies)scene.add(c);gl.render(scene,camera);const candidate=gl.domElement.toDataURL();for(const c of copies){scene.remove(c);c.material.dispose()}gl.render(scene,camera);const restoredEqual=before===gl.domElement.toDataURL();for(const [o,v]of hidden)o.visible=v;scene.remove(invisible);invisible.geometry.dispose();invisible.material.dispose();depthMaterial.dispose();target.dispose();target.depthTexture.dispose();return{position:window.__ember.getWorld().people.find(p=>p.isPlayer)&&[window.__ember.getWorld().people.find(p=>p.isPlayer).x,window.__ember.getWorld().people.find(p=>p.isPlayer).z],matricesExact:copies.every(c=>c.matrix.toArray().every((v,i)=>v===c.userData.sourceMatrix[i])),before,candidate,restoredEqual,unobstructedEqual:before===candidate,meshes:copies.length};
},isolated);
for(const k of ['before','candidate'])fs.writeFileSync(out+'/'+device+'-'+sample+'-'+(isolated?'isolated':'occluded')+'-'+k+'.png',Buffer.from(evidence[k].split(',')[1],'base64'));
result.checks.push({device,sample,position:evidence.position,matricesExact:evidence.matricesExact,isolated,unobstructedEqual:evidence.unobstructedEqual,meshes:evidence.meshes,restoredEqual:evidence.restoredEqual,passed:evidence.meshes>0&&evidence.restoredEqual&&evidence.matricesExact});save();}
await page.waitForFunction(()=>{const p=window.__ember.getWorld().people.find(p=>p.isPlayer);return Math.hypot(p.x-262,p.z-302)<0.2},{},{timeout:15000});result.routes??=[];result.routes.push({device,arrived:true});save();await context.close();

}
result.passed=result.checks.length===12&&result.routes?.length===2&&result.checks.every(x=>x.passed)&&result.errors.length===0;if(!result.passed)process.exitCode=1;
