// Bounded campfire render smoke. Candidate/fallback require Vite dev; built is screenshot-only.
import fs from 'node:fs';
import path from 'node:path';
import assert from 'node:assert/strict';
import {chromium} from 'playwright';
import {SAVE_KEY} from '../src/game/save.ts';
const [url='http://127.0.0.1:8123',label='vis-u4-fire-dev',mode='candidate']=process.argv.slice(2);
...
const result={mode,url,scope:'Campfire geometry render/fallback in controlled arena; desktop/mobile viewport. No crafting/lifetime/contact acceptance.',cases:[],errors:[]};
...
try {
 for(const [device,viewport] of [['desktop',{width:1440,height:960}],['mobile',{width:390,height:844}]]) {
  ...
  if(mode==='fallback')await context.route('**/art/lanternwood/campfire.glb',r=>r.fulfill({status:503,body:'Deliberate local QA rejection'}));
  ...
  for(const weapon of ['campfire']) {
   await page.evaluate(()=>{const w=window.__ember.getWorld(),s=window.__ember.useGame.getState();w.campfires.splice(0,w.campfires.length,{id:'qa-u4-fire',tx:255,ty:303,until:99});s.select(null);s.tick(.01);window.__ember.useGame.setState({snap:{...window.__ember.useGame.getState().snap,campfires:[...w.campfires]}});});await page.waitForTimeout(1600);
   const row={device,weapon};
   if(mode!=='built'){
    row.parts=await page.evaluate(async()=>{const text=await(await fetch('/src/components/game/world-scene.tsx')).text(),f=text.match(/from\s+["']([^"']*(?:react-three_fiber|@react-three\/fiber)[^"']*)["']/),fiber=await import(f[1]);const scene=fiber._roots.get(document.querySelector('canvas')).store.getState().scene;const parts=[];scene.traverse(o=>{if(o.geometry?.userData.campfirePart)parts.push(o.geometry.userData.campfirePart);});return parts.sort();});
    assert.equal(row.parts.length,mode==='fallback'?0:8);
   }
