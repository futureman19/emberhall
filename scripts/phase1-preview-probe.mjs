...
await page.goto(new URL('/art/phase1-preview.html',url).href,{waitUntil:'domcontentloaded'});
await page.evaluate(()=>{localStorage.setItem('emberhall-save-v4','preserve-this-disposable-sentinel');localStorage.removeItem('emberhall-phase1-review-backup-v1')});
await page.locator('#enter').click();await page.waitForURL(u=>u.pathname==='/');
assert.equal(await page.evaluate(()=>JSON.parse(localStorage.getItem('emberhall-phase1-review-backup-v1')).original),'preserve-this-disposable-sentinel');result.checks.push('launcher preserves existing preview save');
await page.goto(new URL('/art/phase1-preview.html',url).href);await page.locator('#restore').click();assert.equal(await page.evaluate(()=>localStorage.getItem('emberhall-save-v4')),'preserve-this-disposable-sentinel');result.checks.push('restore reads back exact prior save');await page.locator('#enter').click();await page.waitForURL(u=>u.pathname==='/');}
...
if(mode!=='reference'){assert(['hall','dormitory','kitchen','yard','market','forge','tavern','notice','board','farm','bank'].every(kind=>result.world.localBuildings.some(b=>b.kind===kind)));for(const name of ['bank','forge','hall','character','oak','kitchen','tavern','market','dormitory','yard','farm','notice','board','interior-hall','interior-dormitory','interior-kitchen','interior-yard','interior-market','interior-forge','interior-tavern','interior-bank']){const response=await page.request.get(new URL(`/art/lanternwood/${name}.glb`,url).href);assert.equal(response.status(),200);const bytes=await response.body();assert.equal(bytes.toString('ascii',0,4),'glTF');result.checks.push(`${name}.glb HTTP200 valid header`)} }
