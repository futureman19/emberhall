...
const out='art/lane3/touch-farm-centered';fs.mkdirSync(out,{recursive:true});assert(!fs.existsSync(out+'/results.json'));
const kinds=['farm'];
const result={scope:'Local8093 renderer, missing-kind disposable worlds, actual touchscreen tap on camera-projected terrain; build mode set by fixture; mobile viewport, not physical phone, physical mobile or animation acceptance.',rows:[],errors:[]};const save=()=>fs.writeFileSync(out+'/results.json',JSON.stringify(result,null,2));
const browser=await chromium.launch({headless:true,args:['--use-angle=d3d11','--enable-gpu']});
try{for(const kind of kinds){
 const c=await browser.newContext({viewport:{width:390,height:844},hasTouch:true});await c.routeWebSocket(/.*/,()=>{});const p=await c.newPage();p.on('pageerror',e=>result.errors.push(e.message));
 ...
 if(!site)throw Error('No site '+kind);you.x=site.tx;you.z=site.ty;you.path=[];s.setPanel('none');s.select(null);window.__ember.useGame.setState({buildKind:kind,buildAt:site});s.tick(0);
 ...
 assert(point.x>0&&point.x<390&&point.y>0&&point.y<844,"Target outside mobile viewport");row.pointer=point;await p.touchscreen.tap(point.x,point.y);await p.waitForTimeout(200);
 row.after=await p.evaluate(async kind=>{const w=window.__ember.getWorld();const {getConstructionFx,constructionPose,CONSTRUCTION_DURATION}=await import("/src/game/construction-animation.ts");return {fx:getConstructionFx(w),activePose:constructionPose(CONSTRUCTION_DURATION*.45),endPose:constructionPose(CONSTRUCTION_DURATION),gold:w.gold,count:w.buildings.length,building:w.buildings.find(b=>b.kind===kind),buildKind:window.__ember.useGame.getState().buildKind};},kind);

 row.commitPassed=row.after.count===setup.count+1&&row.after.gold===100-(kind==='dormitory'?40:28)&&row.after.building?.tx===setup.site.tx&&row.after.building?.ty===setup.site.ty&&row.after.buildKind===null;row.effectContractPassed=row.after.fx?.buildingId===row.after.building?.id&&row.after.fx?.kind===kind&&row.after.activePose.crouch>0&&row.after.endPose.crouch===0;save();assert(row.effectContractPassed,"Effect contract "+kind);assert(row.commitPassed,'Placement commit '+kind);await c.close();
 }result.passed=result.rows.length===1&&result.errors.length===0;
