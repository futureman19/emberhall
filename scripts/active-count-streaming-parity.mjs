...
const label=process.argv[3]??'active-count-streaming-parity';assert.match(label,/^[a-z0-9-]+$/);const out='art/verification/civic/'+label+'.json';assert(!fs.existsSync(out),'Preserve previous results');
...
for(const [device,width,height]of [['desktop',1440,960],['mobile',390,844]]){await page.setViewportSize({width,height});for(const [x,z]of [[256,290],[220,260],[280,320]]){
...
if(proof.images){for(const [key,data]of Object.entries(proof.images))fs.writeFileSync(out+'.'+device+'.'+x+'.'+z+'.'+key+'.png',Buffer.from(data.split(',')[1],'base64'));delete proof.images}result.runs.push({device,x,z,...proof});flush();assert(proof.counts.length>0);assert(proof.pixelsEqual&&proof.restoredEqual&&proof.picksEqual,'pixel/raycast parity');}}
await ctx.close();}
result.passed=result.errors.length===0&&result.runs.length===6;
