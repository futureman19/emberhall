...
try{
const c=await b.newContext();await c.routeWebSocket(/.*/,()=>{});if(label.includes('fallback'))await c.route('**/tools.glb',route=>route.abort());if(label.includes('authored'))await c.route('**/tools.glb',route=>route.fulfill({path:'public/art/lanternwood/tools-silhouette.glb',contentType:'model/gltf-binary'}));const p=await c.newPage();p.setDefaultTimeout(15000);p.on('pageerror',e=>r.errors.push(e.message));p.on('console',m=>{if(m.type()==='error'&&/shader|WebGL/i.test(m.text()))r.errors.push(m.text())});
