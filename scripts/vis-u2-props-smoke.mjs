...
const [url='http://127.0.0.1:8123',label='vis-u2-props-dev',mode='candidate']=process.argv.slice(2);
...
try {
 for(const [device,viewport] of [['desktop',{width:1440,height:960}],['mobile',{width:390,height:844}]]) {
  ...
  if(mode==='fallback')await context.route('**/art/lanternwood/field-props.glb',r=>r.fulfill({status:503,body:'Deliberate local QA rejection'}));
  ...
  if(mode!=='built') {
   row.rendered=await page.evaluate(async()=>{
    ...
    const names=[];scene.traverse(o=>{if(o.name==='authored-field-rocks'||o.name.startsWith('authored-loot-'))names.push(o.name);});return names.sort();
   });
  ...
  const q=new Vector3(254,.81,305).project(camera);const lootPoint={x:view.r.x+(q.x+1)*view.r.width/2,y:view.r.y+(1-q.y)*view.r.height/2};
  if(device==='mobile')await page.touchscreen.tap(lootPoint.x,lootPoint.y);else await page.mouse.click(lootPoint.x,lootPoint.y);
  row.openPile=await page.evaluate(()=>window.__ember.useGame.getState().openPileId);assert.equal(row.openPile,'u2-pile-drop');
