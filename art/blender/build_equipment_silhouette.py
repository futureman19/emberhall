"""Regeneration overwrites manual edits to equipment-silhouette.blend and tools-silhouette.glb.
...
for i,(id,length,width,cy) in enumerate([('hatchet',.42,.045,.16),('pick',.44,.04,.16),('hoe',.44,.04,.16),('fishing_rod',.9,.064,.42)]):
 ...
 if id=='hatchet':o=prism(id+'_head',[(-.1,-.015),(-.025,-.015),(.07,-.05),(.1,-.05),(.1,.05),(.07,.05),(-.025,.015),(-.1,.015)],.066);center=(.08,.36,0)
 elif id=='pick':o=prism(id+'_head',[(-.14,-.035),(-.10,.018),(-.02,.035),(.02,.035),(.10,.018),(.14,-.035),(.078,-.002),(.018,.004),(-.018,.004),(-.078,-.002)],.052);center=(.02,.38,0)
...
bpy.ops.wm.save_as_mainfile(filepath=str(ROOT/'art/blender/equipment-silhouette.blend'))
export=[]
for source in parts:
...
bpy.ops.export_scene.gltf(filepath=str(OUT/'tools-silhouette.glb'),export_format='GLB',use_selection=True,export_apply=True,export_yup=True,export_texcoords=False,export_animations=False)
print('EXPORTED',len(export),'parts', (OUT/'tools-silhouette.glb').stat().st_size,'bytes')
