...
/** Twin front keeps from the hall still — 3×3, merlons, hanging red/gold. */
function hallTower(out: Vox[], x0: number, z0: number) {
  ...
  for (const [x, z] of merlons) put(out, x, top + 1, z, "stone");
  fill(out, x0, top + 2, z0, x1, top + 2, z1, "dark");
  put(out, x0, top + 3, z0, "dark");
  put(out, x1, top + 3, z1, "dark");
  for (const y of [3, 5, 7, 9, 11]) {
    put(out, x0, y, z1, "glass");
    put(out, x1, y, z1, "glass");
    put(out, x0 + 1, y, z1, "glass");
  }
  const pole = x0 < 0 ? x0 : x1;
  for (let y = 4; y <= 11; y++) put(out, pole, y, z1 + 1, "wool");
  put(out, pole, 11, z1 + 1, "gold");
  put(out, pole, 8, z1 + 1, "gold");
}
...
function makeHall(): Spec {
  ...
  const h = 6;
  const out: Vox[] = [];
  fill(out, x0 - 1, 0, z0 - 1, x1 + 1, 0, z1 + 2, "cobble", (x, _y, z) => x >= x0 && x <= x1 && z >= z0 && z <= z1);
  fill(out, x0, 0, z0, x1, 0, z1, "timber");
  fill(out, -2, 0, z1 + 1, 2, 0, z1 + 1, "stone");
  fill(out, -1, 0, z1 + 2, 1, 0, z1 + 2, "stone");
  const holes = new Set<string>();
  for (let x = -1; x <= 1; x++) {
    for (let y = 1; y <= 4; y++) holes.add(key(x, y, z1));
  }
  for (const w of [
    { x: -2, z: z1, w: 1, hh: 2 },
    { x: 2, z: z1, w: 1, hh: 2 },
    ...
    for (let x = w.x; x < w.x + w.w; x++) {
      const bands = w.z === z1 ? [2, 3, 5, 6] : [2, 3];
      for (const y of bands) {
        holes.add(key(x, y, w.z));
        put(out, x, y, w.z, "glass");
      }
      put(out, x, 1, w.z, "gold");
      put(out, x, 4, w.z, "dark");
    }
  }
  fill(out, x0, 1, z0, x1, h, z1, "timber", (x, y, z) => {
    const edge = x === x0 || x === x1 || z === z0 || z === z1;
    if (!edge) return true;
    if ((x <= x0 + 2 && z >= z1 - 2) || (x >= x1 - 2 && z >= z1 - 2)) return true;
    return holes.has(key(x, y, z));
  });
  halfTimber(out, x0, x1, z0, z1, h, z1);
  for (let x = -1; x <= 1; x++) put(out, x, 5, z1, "gold");
  fill(out, -2, 5, z1 + 1, 2, 5, z1 + 1, "wool");
  fill(out, -2, 6, z1 + 1, 2, 6, z1 + 1, "gold");
  put(out, -2, 1, z1, "wool");
  put(out, 2, 1, z1, "wool");
  put(out, -2, 4, z1 + 1, "wool");
  put(out, 2, 4, z1 + 1, "wool");
  hallTower(out, -5, 2);
  hallTower(out, 3, 2);
  gableRoof(out, -2, 2, z0, 1, h, "dark");
