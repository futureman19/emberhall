...
function findSpawnedInputs(): ReadonlyMap<ResourceId, ResourceNodeInput> {
  ...
  const locations = [
    ...
    { tx: 420, ty: 268, nodeKind: "rock" },
    { tx: 110, ty: 440, nodeKind: "rock" },
  ] as const;
...
test("every catalog-spawned definition maps to its catalog node kind and family with bounded finite parameters", () => {
  assert.equal(SPAWNED_IDS.length, 20, "all current catalog definitions that have world spawns are covered");
...
test("terrain keeps selection assignment ordering, animations, and instanced batch count", () => {
  ...
  // Original batches, non-pickable lobes, four same-index oak replacements, and U2's non-pickable authored rock batch.
  assert.equal(source.match(/<instancedMesh\b/g)?.length, 16, "Oak adds four mapped batches; U2 adds one authored rock batch");
